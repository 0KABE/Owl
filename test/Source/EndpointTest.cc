...
TEST(Endpoint, All_Success) {
    ...
    Endpoint endpoint(ioContext.get_executor(), "baidu.com", "80");
...
TEST(Endpoint, All_Failed) {
    using namespace Owl;
    net::io_context ioContext;
    Endpoint endpoint(ioContext.get_executor(), "aaa.com", "80");
    ...
    ioContext.run();
    ASSERT_EQ(endpoint.GetState(), Endpoint::ConnectionState::DISCONNECTED);
}
